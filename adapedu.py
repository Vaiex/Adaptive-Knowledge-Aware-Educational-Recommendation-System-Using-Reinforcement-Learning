...
# ── Student Simulator ──────────────────────────────────────────────────────────
#
# Learning rates grounded in Koedinger et al. (PNAS 2023): students show low
# variance in learning rate per opportunity; variation is mostly in initial
# knowledge. We spread profiles across a plausible ±2x range around a centre
# of ~0.10 (≈10% gain per well-matched interaction).
#
# Forgetting rates grounded in empirical retention studies (ERIC, 2022):
# ~3% loss per week at the topic level. Translating to per-step (one step ≈ one
# short session), fast learners consolidate better (lower decay); struggling
# students show faster decay consistent with weaker encoding.
...
# Content-type multipliers encode documented pedagogical patterns:
#   tutorial  — highly effective for low mastery (scaffolding), weak for high mastery
#   challenge — effective only once mastery is sufficient; harmful below threshold
#   quiz      — consolidates knowledge above ~40%; demoralises below it
#   video     — baseline; useful at any level but not optimal at any level
CONTENT_MULTIPLIERS = {
    "video":     lambda k: 0.90,
...
class StudentSimulator:
    ...
    def step(self, topic, content_type, difficulty):
        ...
        # ZPD factor — max gain when difficulty matches mastery exactly (Murray & Arroyo 2002)
        zpd_factor = max(0.0, 1.0 - abs(difficulty - k) / ZPD_TOLERANCE)

        # Content-type multiplier (pedagogical effectiveness)
        ...
        # Engagement: drops off sharply when content is far outside ZPD
        engagement = 1.0 if abs(difficulty - k) <= ZPD_TOLERANCE else 0.30

        # Knowledge gain formula from paper:
        ...
        # Struggling students get a boost from scaffolding content, penalty from challenge/quiz
        ...
        # Decay ∝ exp(-k*2): weakly encoded knowledge decays faster
...
class LearningEnv:
    ...
    def step(self, action_idx):
        ...
        # R = 3.0*ΔK + 1.5*E + 2.0*ΔK_total + B_mastery - 0.8*I_redundant
        #     - λ * max(0, |d - k_τ| - δ_ZPD)
